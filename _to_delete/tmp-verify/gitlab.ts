...
export const GitLabMergeRequestSummarySchema = z
  .object({
    ...
    // Known values: "opened" | "closed" | "locked" | "merged". Kept open-ended so an
    // unknown state from a newer GitLab version cannot drop the entire MR list.
    state: z.union([
      z.enum(["opened", "closed", "locked", "merged"]),
...
/**
 * Known values are "new" and "old", but GitLab also reports "expanded" for unchanged
 * context lines inside multi-line comments, and omits the field or sends null on some
 * instances/versions. A single unexpected value must never invalidate the whole
 * discussion list, so this stays a bounded free-form string.
 */
export const GitLabLineRangeSideTypeSchema = z
  .string()
  .trim()
  .max(50)
  .nullable()
...
const GitLabLineRangeSideSchema = z
  .object({
    lineCode: z.string().trim().max(255).nullable().optional(),
    type: GitLabLineRangeSideTypeSchema,
...
export const GitLabLineRangeSchema = z
  .object({
    start: GitLabLineRangeSideSchema,
    end: GitLabLineRangeSideSchema,
  })
  .strict();

/** A commit SHA, or "" when GitLab omits the ref (the mapper normalizes missing refs to ""). */
const OptionalShaSchema = z.union([ShaSchema, z.literal("")]);

export const GitLabDiffPositionSchema = z
  .object({
    positionType: z.enum(["text", "image", "file"]),
    baseSha: OptionalShaSchema,
    startSha: OptionalShaSchema,
    headSha: OptionalShaSchema,
...
export type GitLabLineRangeSideType = z.infer<typeof GitLabLineRangeSideTypeSchema>;
