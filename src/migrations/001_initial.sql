-- GeminUI schema v1. This is the single, fresh-install equivalent of the
-- consolidated desktop schema that replaces the former incremental migrations.
...
CREATE TABLE sessions (
  ...
  available_modes_json TEXT NOT NULL DEFAULT '[]'
...
CREATE TABLE context_attachment_links (
  ...
  preview_description TEXT CHECK(preview_description IS NULL OR length(preview_description) <= 1000),
  preview_site_name TEXT CHECK(preview_site_name IS NULL OR length(preview_site_name) <= 200),
  preview_image_file TEXT CHECK(preview_image_file IS NULL OR length(preview_image_file) > 0),
...
CREATE TABLE gitlab_repository_bindings (
  ...
  source_project_path TEXT NOT NULL CHECK(length(source_project_path) BETWEEN 1 AND 1024),
  enabled INTEGER NOT NULL DEFAULT 0 CHECK(enabled IN (0, 1)),
  selected_target_project_id INTEGER CHECK(selected_target_project_id > 0),
  selected_target_project_path TEXT CHECK(selected_target_project_path IS NULL OR length(selected_target_project_path) BETWEEN 1 AND 1024),
