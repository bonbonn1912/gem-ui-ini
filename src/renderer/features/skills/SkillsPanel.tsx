...
export function SkillsPanel({ projectId, onClose }: SkillsPanelProps) {
  const [list, setList] = useState<GeminiSkillList | null>(null);
  ...
  return (
    <aside className="extension-panel skills-panel" aria-label="Gemini-Skills">
      <header className="extension-panel-header skills-panel-header">
        <div>
          <span className="extension-panel-icon skills-panel-icon">
        ...
        <div className="extension-panel-scroll">
          ...
          {loading && !list && (
            <div className="extension-loading">
              <span className="mini-spinner" /> Skills werden gelesen …
            </div>
          )}
          ...
          {filtered.map((skill) => (
            <article
              className={`extension-row ${skill.enabled ? "" : "extension-row--off"}`}
              key={skill.id}
            >
              <header>
                <strong title={skill.name}>{skill.name}</strong>
                <span className={`extension-badge extension-badge--${skill.scope}`}>
                  {scopeLabel(skill)}
                </span>
                {!skill.enabled && (
