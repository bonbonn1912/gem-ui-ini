...
type UseGitChangePreviewsInput = {
  ...
  trigger: GitPreviewTrigger | null;
};
...
export function useGitChangePreviews({
  ...
  status,
  trigger,
}: UseGitChangePreviewsInput): ReadonlyMap<string, GitPreviewGroup> {
  const [groups, setGroups] = useState<ReadonlyMap<string, GitPreviewGroup>>(
    () => new Map(),
  );

  useEffect(() => {
    setGroups(new Map());
  }, [sessionId, project?.id, project?.rootRevision]);

  useEffect(() => {
    if (!trigger) return;
    setGroups((current) => {
      const next = new Map(current);
      next.set(trigger.toolCallId, {
  ...
  useEffect(() => {
    if (!project || !status || !trigger) return;
    if (status.projectId !== project.id) return;
    if (status.refreshedAt === trigger.statusRefreshedAt) return;

    const candidates = status.changes.filter((change) =>
      trigger.baseline.get(changeKey(change)) !== change.fileId,
    );
    if (candidates.length === 0) {
      setGroups((current) => {
        const next = new Map(current);
        next.delete(trigger.toolCallId);
        return next;
      });
      return;
    }
    ...
    const load = async () => {
      ...
      for (const change of candidates.slice(0, MAX_PREVIEW_FILES)) {
        ...
        try {
          ...
          const repository = status.repositories.find((candidate) =>
  ...
  }, [project, status?.refreshedAt, trigger]);
